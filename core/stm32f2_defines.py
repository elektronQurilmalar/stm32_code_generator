# --- NEW FILE core/stm32f2_defines.py ---
...
PLLN_MIN_GENERAL = 192;
PLLN_MAX_GENERAL = 432  # Bits 14:6 (Note: some sources say 50-432, RM0033 for F20x says 192-432 for VCO_OUT range directly related to N)
# Effective range for N will be to keep VCO_OUT in range.
...
VCO_OUTPUT_MIN_HZ = 192000000;
VCO_OUTPUT_MAX_HZ = 432000000  # Main PLL VCO output (fVCO)
...
AHB_PRESCALER_MAP_F2 = {  # HPRE bits 7:4
    1: 0b0000, 2: 0b1000, 4: 0b1001, 8: 0b1010,
    16: 0b1011, 64: 0b1100, 128: 0b1101, 256: 0b1110,
    512: 0b1111
}
APB_PRESCALER_MAP_F2 = {  # PPRE1 bits 12:10, PPRE2 bits 15:13
...
FLASH_LATENCY_F2_MAP = {
    ...
    # For VDD 2.4-2.7V, max is 96MHz, latencies shift
    # For VDD 2.1-2.4V, max is 72MHz
    # For VDD 1.8-2.1V, max is 60MHz
    # This map assumes typical VDD range for max speed.
}
...
def get_f2_flash_latency(hclk_freq_hz, target_device_id,
    ...
    latency_map_to_use = FLASH_LATENCY_F2_MAP  # Default for VDD 2.7-3.6V
    if device_info and "flash_latency_config" in device_info:
        latency_map_to_use = device_info["flash_latency_config"]

    # Logic from F4 get_flash_latency is generally good
    ...
    for ws, (min_hz_excl, max_hz_incl) in latency_map_to_use.items():
        if hclk_mhz_int > min_hz_excl and hclk_mhz_int <= max_hz_incl:
            return ws
    # If HCLK is 0 or very low (e.g. during init before PLL)
    if hclk_mhz_int <= latency_map_to_use.get(0, (0, 0))[1]: return 0

    return max(latency_map_to_use.keys()) if latency_map_to_use else 3  # Fallback to max WS for 120MHz
...
RCC_CR_PLLI2SRDY_Pos = 27;  # PLLI2S for I2S/SAI
RCC_CR_PLLSAION_Pos = 28;
RCC_CR_PLLSAIRDY_Pos = 29;  # PLLSAI for SAI/LCD (F4 specific, F2 doesn't have PLLSAI) - Remove for F2
...
FLASH_ACR_LATENCY_Msk = 0xF;  # Max 7 WS for F2, but 3 WS for max speed at typical VDD
FLASH_ACR_PRFTEN_Pos = 8;
FLASH_ACR_PRFTEN = (1 << FLASH_ACR_PRFTEN_Pos);  # Prefetch enable
FLASH_ACR_ICEN_Pos = 9;
FLASH_ACR_ICEN = (1 << FLASH_ACR_ICEN_Pos);  # Instruction cache enable
FLASH_ACR_DCEN_Pos = 10;
FLASH_ACR_DCEN = (1 << FLASH_ACR_DCEN_Pos);  # Data cache enable
FLASH_ACR_ICRST_Pos = 11;
FLASH_ACR_ICRST = (1 << FLASH_ACR_ICRST_Pos);  # Instruction cache reset
FLASH_ACR_DCRST_Pos = 12;
FLASH_ACR_DCRST = (1 << FLASH_ACR_DCRST_Pos);  # Data cache reset

# PWR_CR - F2 does NOT have VOS (Voltage Scaling) like F4. Max speed is determined by VDD.
# It has PLS (PVD level selection) and PVDE (PVD enable).
RCC_APB1ENR_PWREN_Pos = 28;
RCC_APB1ENR_PWREN = (1 << RCC_APB1ENR_PWREN_Pos)

# --- GPIO Defines for F2 (Similar to F4) ---
# MODER, OTYPER, OSPEEDR, PUPDR, AFRx registers are present.
GPIO_MAX_PORT_CHAR_F2 = 'I'  # STM32F2xx can go up to GPIOI

# --- ADC Defines for F2 (Similar to F4 for basic setup) ---
ADC_CHANNELS_MAP_F2 = {  # ADC1/2/3 have 16 ext channels
    "STM32F205VC": {f"IN{i}": i for i in range(16)} | {"TEMP": 16, "VREFINT": 17, "VBAT": 18},
    "STM32F207VG": {f"IN{i}": i for i in range(16)} | {"TEMP": 16, "VREFINT": 17, "VBAT": 18},
}
# ADC_CCR bits (ADCPRE is in CCR for F2)
ADC_CCR_ADCPRE_Pos = 16;
ADC_CCR_ADCPRE_Msk = (0x3 << ADC_CCR_ADCPRE_Pos)  # Values 00:PCLK2/2, 01:PCLK2/4, 10:PCLK2/6, 11:PCLK2/8
ADC_CCR_VBATE_Pos = 22;
ADC_CCR_VBATE = (1 << ADC_CCR_VBATE_Pos)
ADC_CCR_TSVREFE_Pos = 23;
ADC_CCR_TSVREFE = (1 << ADC_CCR_TSVREFE_Pos)
ADC_CCR_MULTI_Pos = 0;
ADC_CCR_MULTI_Msk = (0x1F << ADC_CCR_MULTI_Pos)  # Multi ADC mode
# Other ADC register bits (CR1, CR2, SQRx, SMPRx) are largely similar to F4.
...
# Bit positions for CR1, CR2, CR3, BRR, SR are mostly the same as F4.
# F2 USART does NOT have OVER8 bit in CR1; it's always oversampling by 16.
USART_OVERSAMPLING_MAP_F2 = {"16": 0}  # Only x16 supported

# --- Timer Defines for F2 ---
# TIM1/8 Advanced, TIM2/5 32-bit GP, TIM3/4 16-bit GP, TIM6/7 Basic, TIM9-14 16-bit GP.
TIMER_PERIPHERALS_INFO_F2 = {
    "TIM1": {"type": "ADV", "bus": "APB2", "rcc_macro": "RCC_APB2ENR_TIM1EN", "max_channels": 4, "has_bdtr": True,
             "is_16bit": True},
    "TIM2": {"type": "GP32", "bus": "APB1", "rcc_macro": "RCC_APB1ENR_TIM2EN", "max_channels": 4, "has_bdtr": False,
             "is_16bit": False},  # 32-bit counter
    "TIM3": {"type": "GP16", "bus": "APB1", "rcc_macro": "RCC_APB1ENR_TIM3EN", "max_channels": 4, "has_bdtr": False,
             "is_16bit": True},
    "TIM4": {"type": "GP16", "bus": "APB1", "rcc_macro": "RCC_APB1ENR_TIM4EN", "max_channels": 4, "has_bdtr": False,
             "is_16bit": True},
    "TIM5": {"type": "GP32", "bus": "APB1", "rcc_macro": "RCC_APB1ENR_TIM5EN", "max_channels": 4, "has_bdtr": False,
             "is_16bit": False},  # 32-bit counter
    "TIM6": {"type": "BASIC", "bus": "APB1", "rcc_macro": "RCC_APB1ENR_TIM6EN", "max_channels": 0, "has_bdtr": False,
             "is_16bit": True},
    "TIM7": {"type": "BASIC", "bus": "APB1", "rcc_macro": "RCC_APB1ENR_TIM7EN", "max_channels": 0, "has_bdtr": False,
             "is_16bit": True},
    "TIM8": {"type": "ADV", "bus": "APB2", "rcc_macro": "RCC_APB2ENR_TIM8EN", "max_channels": 4, "has_bdtr": True,
             "is_16bit": True},
    "TIM9": {"type": "GP16", "bus": "APB2", "rcc_macro": "RCC_APB2ENR_TIM9EN", "max_channels": 2, "has_bdtr": False,
             "is_16bit": True},
    "TIM10": {"type": "GP16", "bus": "APB2", "rcc_macro": "RCC_APB2ENR_TIM10EN", "max_channels": 1, "has_bdtr": False,
              "is_16bit": True},
    "TIM11": {"type": "GP16", "bus": "APB2", "rcc_macro": "RCC_APB2ENR_TIM11EN", "max_channels": 1, "has_bdtr": False,
              "is_16bit": True},
    "TIM12": {"type": "GP16", "bus": "APB1", "rcc_macro": "RCC_APB1ENR_TIM12EN", "max_channels": 2, "has_bdtr": False,
              "is_16bit": True},
    "TIM13": {"type": "GP16", "bus": "APB1", "rcc_macro": "RCC_APB1ENR_TIM13EN", "max_channels": 1, "has_bdtr": False,
              "is_16bit": True},
    "TIM14": {"type": "GP16", "bus": "APB1", "rcc_macro": "RCC_APB1ENR_TIM14EN", "max_channels": 1, "has_bdtr": False,
              "is_16bit": True},
}
# Timer register bits are largely similar to F4.
...
# I2C register bits are largely similar to F4. Min PCLK for F2 I2C is 2MHz.
...
SPI_PERIPHERALS_INFO_F2 = {
    ...
    "SPI2": {"bus": "APB1", "rcc_macro": "RCC_APB1ENR_SPI2EN"},  # Can be I2S2
    "SPI3": {"bus": "APB1", "rcc_macro": "RCC_APB1ENR_SPI3EN"},  # Can be I2S3
    # F21x may have SPI4/5/6. Add if supporting those.
}
# SPI register bits are largely similar to F4.

# --- DMA Defines for F2 ---
# F2 has DMA1 and DMA2, each with 8 streams. Each stream maps to one of 8 channels (mux).
# This is very similar to F4's DMA structure.
DMA_PERIPHERALS_INFO_F2 = {
    "DMA1": {"rcc_macro": "RCC_AHB1ENR_DMA1EN", "streams": 8},
    "DMA2": {"rcc_macro": "RCC_AHB1ENR_DMA2EN", "streams": 8}
}
# Register map (SxCR, SxFCR, SxPAR, SxM0AR, SxNDTR, LISR, HISR, etc.)
# and bit positions are largely identical to STM32F4.

# --- DAC Defines for F2 ---
DAC_PERIPHERALS_INFO_F2 = {
    "DAC1": {"rcc_macro": "RCC_APB1ENR_DACEN", "channels": 2},  # DAC block has DAC_CH1 and DAC_CH2
}
DAC_OUTPUT_PINS_F2 = {  # From datasheet for typical packages
    "STM32F205VC": {"DAC_OUT1": "PA4", "DAC_OUT2": "PA5"},
    "STM32F207VG": {"DAC_OUT1": "PA4", "DAC_OUT2": "PA5"},
}
# DAC register bits (CR, SWTRIGR, DHRx) are very similar to F4.